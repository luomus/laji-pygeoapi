...
def load_and_process_data(occurrence_url, table_base_name, pages, config, all_value_ranges, taxon_df, collection_names, municipal_geojson_path, lookup_table):
    """
...
def main():
    ...
    if config['pages_env'] == '0':
        ...
        for province_id in biogeographical_province_ids:
            ...
            results = load_and_process_data(occurrence_url, table_base_name, pages, config, all_value_ranges, taxon_df, collection_names, 'municipalities.geojson', 'lookup_table_columns.csv')
        ...
        if os.getenv("INVASIVE_SPECIES", "True").lower() == "true":
            ...
            pages = load_data.get_last_page(occurrence_url) if config["pages_env"] in ["all", "latest"] else int(config["pages_env"])
            results = load_and_process_data(occurrence_url, 'invasive_species', pages, config, all_value_ranges, taxon_df, collection_names, 'municipalities.geojson', 'lookup_table_columns.csv')
